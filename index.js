...
app.post("/submit", (req, res) => {
  ...
  const name1 = Math.floor(Math.random()*adj.length);
  const name2 = Math.floor(Math.random()*noun.length);
  const word = adj[name1] +" "+ noun[name2];
  const year = new Date().getFullYear()
  res.render("index.ejs",{finalWord : word, newYear : year});
});
